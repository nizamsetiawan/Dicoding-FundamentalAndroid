...
class FollowersViewModel : ViewModel() {
    private val _listFollowers = MutableLiveData<List<ItemResponse>>()
    val listFollowers: LiveData<List<ItemResponse>> = _listFollowers
        ...
        call.enqueue(object : Callback<List<ItemResponse>> {
            override fun onResponse(call: Call<List<ItemResponse>>, response: Response<List<ItemResponse>>) {
                ...
                if (response.isSuccessful) {
                    ...
                    _listFollowers.postValue(followers?.toList())
                } else {
